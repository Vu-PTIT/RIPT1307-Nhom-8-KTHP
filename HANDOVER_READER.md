# Bàn giao công việc Reader Flow
...
## 9) Update mới vừa thực hiện

- `backend/init_db.py`: seed thêm role `Librarian` và demo users `admin`, `library`, `test123` nếu chưa tồn tại.
- `backend/app/crud/borrow.py`: thêm helper `_resolve_reference_id` để xử lý reference id an toàn, tránh dùng `.id` trực tiếp trên đối tượng tham chiếu không chắc chắn.
- `base-web-umi/src/pages/TaiLieu/index.tsx`: hiện thực trang tài liệu với tìm kiếm, thêm vào yêu thích, thêm vào giỏ mượn.
- `base-web-umi/src/pages/YeuThich/index.tsx`: hiện thực trang yêu thích với xóa và chuyển sang giỏ mượn.
- `base-web-umi/src/pages/GioMuon/index.tsx`: hiện thực trang giỏ mượn với xóa item và xóa toàn bộ.
- `base-web-umi/src/pages/LichSuMuon/index.tsx`: hiện thực trang lịch sử mượn với bảng mở rộng xem chi tiết item.
- `base-web-umi/src/pages/GiaHan/index.tsx`: hiện thực trang gia hạn với modal gửi yêu cầu gia hạn từ danh sách item đang mượn.
- `base-web-umi/src/pages/DiemDanh/index.tsx`: hiện thực trang check-in/check-out với lịch sử điểm danh.
...
- Đã bắt đầu sửa backend reader flow để tránh sử dụng `.model` trên các reference.
- Các file `wishlist.py`, `borrow_cart.py`, `borrows.py`, `renewals.py`, `deps.py` đã được cập nhật.
